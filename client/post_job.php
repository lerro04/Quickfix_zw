...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(isset($_FILES['job_images'])){
        $upload = processImageUploads($_FILES['job_images'], 'jobs');
        foreach($upload['uploads'] as $file){
            $pdo->prepare("INSERT INTO job_request_images (job_id,image_path,original_name,mime_type,file_size) VALUES (?,?,?,?,?)")->execute([$jobId,$file['path'],$file['original_name'],$file['mime_type'],$file['file_size']]);
        }
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select name="trade" class="form-select" required>
            ...
            <?php foreach($TRADES as $t): ?><option value="<?=$t?>"><?=$t?></option><?php endforeach; ?>
          </select>
        </div>
        <div class="form-group">
