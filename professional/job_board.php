...
$prof = $pdo->prepare("SELECT * FROM professional_profiles WHERE user_id=?");
$prof->execute([$uid]);
$p = $prof->fetch();
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_bid'])){
    if(!$u['verified']){
        $msg = 'Your account must be verified before you can bid.';
    } else {
        $jobId = (int)$_POST['job_id'];
        $amount = (float)$_POST['bid_amount'];
        $message = htmlspecialchars(trim($_POST['bid_message']));
        $days = (int)$_POST['estimated_days'];
        $ex = $pdo->prepare("SELECT bid_id FROM bids WHERE job_id=? AND professional_id=?");
        $ex->execute([$jobId, $uid]);
        if($ex->fetch()){
            $msg = 'You already placed a bid on this job.';
        } else {
            $pdo->prepare("INSERT INTO bids (job_id,professional_id,bid_amount,message,estimated_days) VALUES (?,?,?,?,?)")->execute([$jobId,$uid,$amount,$message,$days]);
            $msg = 'Bid placed successfully. You will be notified if the client accepts.';
        }
    }
}
...
<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;margin-bottom:1.2rem">
  ...
  <div style="display:flex;gap:0.5rem;flex-wrap:wrap">
    ...
    <a href="?filter=urgent" class="btn btn-<?=$filter==='urgent'?'danger':'outline'?> btn-sm"><?=icon('triangle-exclamation')?> Urgent Only</a>
  </div>
</div>
...
<?php $jobImages = $jobImagesById[$j['job_id']] ?? []; ?>
<div class="job-card">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:1rem">
    <div style="flex:1">
      ...
      <div class="job-meta">
        ...
        <span><?=icon('location-dot')?> <?=$j['location']?></span>
        <span><?=icon('user')?> <?=htmlspecialchars($j['client_name'])?></span>
        <span><?=icon('clock')?> <?=timeAgo($j['created_at'])?></span>
        <span><?=urgencyBadge($j['urgency'])?></span>
      </div>
      ...
      <?php if(!empty($jobImages)): ?>
        <div class="job-image-strip">
          <?php foreach(array_slice($jobImages, 0, 4) as $image): ?>
  ...
  <div style="margin-top:0.8rem;display:flex;gap:0.5rem;flex-wrap:wrap">
    ...
    <?php endif; ?>
    <a href="/quickfix/messages.php?with=<?=$j['client_id']?>" class="btn btn-outline btn-sm"><?=icon('comments')?> Message Client</a>
  </div>
</div>
<?php endforeach; ?>
...
<div class="modal-overlay" id="bid-modal">
  <div class="modal-box">
    <div class="modal-header">
      <?=icon('sack-dollar')?> Place Your Bid
      <span class="modal-close" onclick="document.getElementById('bid-modal').classList.remove('show')">&times;</span>
    </div>
    <div class="modal-body">
      ...
      <form method="POST">
        ...
        <button type="submit" class="btn btn-primary btn-block"><?=icon('paper-plane')?> Submit Bid</button>
      </form>
    </div>
  </div>
</div>
