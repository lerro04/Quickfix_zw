  ...
  <div>
    ...
    <div class="bid-card <?=$b['status']==='accepted'?'accepted':''?>">
      ...
      <div style="display:flex;gap:0.5rem;margin-top:0.8rem">
        <form method="POST"><input type="hidden" name="bid_id" value="<?=$b['bid_id']?>"><button name="accept_bid" class="btn btn-success btn-sm" onclick="return confirm('Accept this bid? A booking will be created.')"><?=icon('check')?> Accept Bid</button></form>
        <form method="POST"><input type="hidden" name="bid_id" value="<?=$b['bid_id']?>"><button name="reject_bid" class="btn btn-danger btn-sm"><?=icon('xmark')?> Reject</button></form>
        <a href="/quickfix/messages.php?with=<?=$b['professional_id']?>" class="btn btn-outline btn-sm"><?=icon('comments')?> Message</a>
      </div>
      <?php endif; ?>
    </div>
    <?php endforeach; ?>
