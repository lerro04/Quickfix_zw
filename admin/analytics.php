...
$todayPlatformRevenue = (float)$pdo->query("SELECT COALESCE(SUM(CASE WHEN platform_fee_amount IS NULL OR platform_fee_amount = 0 THEN ROUND(agreed_amount * ".(PLATFORM_COMMISSION_RATE * 100)." / 100, 2) ELSE platform_fee_amount END),0) as t FROM bookings WHERE payment_status='released' AND DATE(completed_at)=CURDATE()")->fetch()['t'];
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem">
 ...
 <span><?=$t['bookings']?> bookings &middot; <span style="color:var(--success)">$<?=number_format($t['revenue'],2)?></span></span>
 </div>
