...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['start_job'])){
    ...
    $msg="âœ… Job started! Complete the work and the client will mark it as done.";
}
...
<div class="card-grid" style="margin-bottom:1.5rem">
  <div class="stat-card" style="border-left-color:var(--success)"><div class="stat-icon" style="background:var(--success)">ðŸ’µ</div><div class="stat-info"><h3>$<?=number_format($earned,2)?></h3><p>Total Earned</p></div></div>
  <div class="stat-card" style="border-left-color:var(--warning)"><div class="stat-icon" style="background:var(--warning)">â³</div><div class="stat-info"><h3>$<?=number_format($pendingAmt,2)?></h3><p>Pending Payment</p></div></div>
</div>
<div class="page-title">ðŸ“… My Bookings (<?=count($bookings)?>)</div>
...
<div class="job-card">
  <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:0.8rem;margin-bottom:0.8rem">
    <div>
      ...
      <div class="job-meta">
        <span>ðŸ‘¤ <?=$b['client_name']?></span>
        <span>ðŸ“ž <?=$b['client_phone']?></span>
        <span>ðŸ“… <?=$b['scheduled_date']??'Date TBD'?></span>
      </div>
  ...
  <div style="display:flex;gap:0.5rem;flex-wrap:wrap">
    <form method="POST"><input type="hidden" name="booking_id" value="<?=$b['booking_id']?>"><button name="start_job" class="btn btn-primary btn-sm">ðŸš€ Start Job</button></form>
    <a href="<?= BASE_URL ?>/messages.php?with=<?=$b['client_id']?>" class="btn btn-outline btn-sm">ðŸ’¬ Message Client</a>
  </div>
  ...
  <div style="padding:0.6rem;background:#fff3cd;border-radius:8px;font-size:0.85rem;color:#856404">
    ðŸ”§ Job in progress. The client will mark it complete once you're done. <a href="<?= BASE_URL ?>/messages.php?with=<?=$b['client_id']?>" style="color:var(--primary)">Message client â†’</a>
  </div>
  <?php elseif($b['status']==='completed'): ?>
  <div style="padding:0.6rem;background:#d4edda;border-radius:8px;font-size:0.85rem;color:#155724">
    âœ… Job completed. Payment <?=$b['payment_status']==='released'?'has been released to you':'is being processed'?>.
  </div>
  <?php endif; ?>
</div>
<?php endforeach; ?>
