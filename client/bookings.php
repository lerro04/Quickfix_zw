...
<?php if($msg): ?><div class="alert alert-<?=htmlspecialchars($msgType)?>"><?=icon($msgType==='success'?'circle-check':'triangle-exclamation')?> <?=htmlspecialchars($msg)?></div><?php endif; ?>
...
<div class="job-card">
  <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:0.5rem;margin-bottom:0.8rem">
    <div>
      ...
      <div class="job-meta">
        ...
        <span><?=icon('calendar-day')?> <?=$b['scheduled_date'] ?? 'TBD'?></span>
      </div>
    </div>
  ...
  <div style="display:flex;gap:0.5rem;flex-wrap:wrap;align-items:center">
    <?php if($b['payment_status']==='pending'): ?>
      <form method="POST" action="<?= BASE_URL ?>/paynow_init.php">
        ...
        <button type="submit" class="btn btn-primary btn-sm"><?=icon('credit-card')?> Pay $<?=number_format($b['agreed_amount'],2)?> via Paynow</button>
      </form>
    <?php elseif($b['payment_status']==='held'): ?>
      <span class="badge badge-success"><?=icon('credit-card')?> Paid &mdash; funds held until you confirm completion</span>
    <?php endif; ?>
    <form method="POST" onsubmit="return confirm('Mark as complete and release payment?')">
      <input type="hidden" name="booking_id" value="<?=$b['booking_id']?>">
      <button name="complete" class="btn btn-success btn-sm"><?=icon('circle-check')?> Mark Complete &amp; Release Payment</button>
    </form>
    <form method="POST">
      <input type="hidden" name="booking_id" value="<?=$b['booking_id']?>">
      <button name="dispute" class="btn btn-danger btn-sm"><?=icon('triangle-exclamation')?> Raise Dispute</button>
    </form>
  ...
  <div style="margin-top:0.8rem;border-top:1px solid var(--border);padding-top:0.8rem">
    ...
    <form method="POST" style="margin-top:0.5rem;display:flex;gap:0.5rem;flex-wrap:wrap;align-items:flex-end">
      ...
      <select name="rating" class="form-select" style="width:200px">
